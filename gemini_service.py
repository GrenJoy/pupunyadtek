...
КРИТИЧЕСКИ ВАЖНО для определения today/tomorrow:
1. Если в тексте поста указана конкретная дата (например, "06.12", "6 грудня", "06.12.2025"):
   - Дата в тексте = {today_date_str} → full_today или partial_today
   - Дата в тексте = {tomorrow_date_str} → full_tomorrow или partial_tomorrow
   - Дата в тексте < {today_date_str} → ignore (старая дата)
   
2. Если в тексте НЕТ конкретной даты, но ЕСТЬ интервалы времени (например, "с 21:00 до 24:00", "21-24", "до 24:00"):
   - Анализируй ВРЕМЯ НАЧАЛА интервала:
     * Если интервал начинается СЕГОДНЯ (например, "с 21:00" когда пост в 20:00, или "21:00-24:00" когда пост в 20:00) → partial_today
     * Если интервал начинается ЗАВТРА (например, "с 00:00", "с 01:00", "00:00-03:00") → partial_tomorrow
   - Примеры:
