...
- Если ≥6 групп (1.1, 2.1 и т.д.) с интервалами И НЕТ слов "зміни", "включення", "подовжено", "додатково" → full_today или full_tomorrow (зависит от даты)
- Если есть "графік", "ГПВ", "черга" И НЕТ слов "зміни" → full_today или full_tomorrow (зависит от даты)
...
1. ОБЪЕДИНЕНИЕ ИНТЕРВАЛОВ (самое важное!):
   ...
   - ВАЖНО: В ответе верни ТОЛЬКО группы, которые упомянуты в новом сообщении, но с ОБЪЕДИНЕННЫМИ интервалами (старые + новые)
...
Правильный результат: {{"1.1": ["06:00-07:00", "13:30-17:00"], "2.1": ["09:00-12:00"]}}
